...
def makeCPseriesFile(cpseriesfilename, fluorFiles, appendLibData=None):
    """For one tile, starting with CPfluor files, combine to get CPseries.
    Args:
        cpseriesfilename - output cpseries file
        fluorFiles - List[str], list of fluor files for a given tile
    """
    signals = []
    ...
    # assert(len(identifiers) == len(fluorFiles_list))
    ...
    for i, (idx, filename) in enumerate(fluorFiles.items()):
        ...
        if os.path.exists(filename):
            ...
            try:
                signals.append(pd.Series(getSignalFromCPFluor(filename), name=os.path.dirname(filename).split('/')[-1]))
            except IndexError as error:
                print('IndexError!!!', error)
                print('idx', idx)
                print('fluorefile filename:', filename)
                print('identifiers:', identifiers)
                print('fluorFiles:', fluorFiles)
    ...
    if not appendLibData is None:
        ...
        signal = signal.merge(libData[['RefSeq','SEQID', 'Series']],on='clusterID',how='left')
