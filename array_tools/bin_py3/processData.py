...
group.add_argument('--appendLibData', help='Append all columns from this file to final output')
...
print('fluorNamesByTileDict', fluorNamesByTileDict)
...
#signalNamesByTileDict = processing.getSimpleCPseriesDictfromFluorDict(fluorNamesByTileDict, signalDirectory)
...
for directory, fluorDirs, outputFiles in zip([signalDirectory, ],
                                             [fluorNamesByTileDict, ],
                                             [signalNamesByTileDict, ]):
    print('Converting CPfluor files into CPseries files in directory "%s"'%directory)
    # get outputs
    tileList = np.sort(list(fluorDirs.keys()))
    print(tileList)
...
if os.path.exists(reducedCPsignalFile): #and os.path.exists(reducedRedCPsignalFile):
    ...
    if fileWithIndexToKeep is None:
        if args.filter_pos is None:
            ...
            indices = None
        if args.appendLibData is not None:
            # make file using indices from the appendLibData file

            call = "cut -f1 %s | tail -n +2 > tmp_indices.CPseq" %(args.appendLibData)

            print(call)
            os.system(call)

            indices = fileio.loadFile('tmp_indices.CPseq').index
            #indices = [x.replace('@','').replace(' 1:N:0:1','') for x in indices]
            ...
            filteredCPseqFilenameDict = processing.findTileFilesInDirectory(args.filtered_CPse0qs,
                                                                        ['CPseq'])
    ...
    for outputFiles, reducedOutputFile in zip([signalNamesByTileDict,],
                                                         [reducedCPsignalFile,]):
        print('\tSaving to: %s'%reducedOutputFile)
        if not alreadySavedTiles:
            if args.appendLibData is not None:
                print('Appending existing lib data')
                processing.reduceCPseriesFiles(outputFiles, reducedOutputFile, indices, tileOutputFile, appendLibData=args.appendLibData)
            ...
            if args.appendLibData is not None:
                print('HI Appending existing lib data')
                processing.reduceCPseriesFiles(outputFiles, reducedOutputFile, indices, appendLibData=args.appendLibData) 
            else:
